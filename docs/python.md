...
Supported model names are `silero`, `pyannote`, `pulsevad`, `fsmn`, `ten`, and `marblenet`. `silero` and `silero-v6` select Silero VAD v6; use `silero-v5` for the retained v5 model. Pass `quantized=True` for the PulseVAD, FSMN, or MarbleNet INT8 model. Candle supports the FSMN INT8 graph by dequantizing its weights at load time; TEN and the other INT8 variants require `runtime="onnxruntime"`.
