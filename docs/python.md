...
Supported model names are `silero`, `pyannote`, `pulsevad`, `fsmn`, `ten`, and `marblenet`. `silero` and `silero-v6` select Silero VAD v6; use `silero-v5` for the retained v5 model. Pass `quantized=True` for the PulseVAD, FSMN, or MarbleNet INT8 model. Candle supports the PulseVAD and FSMN INT8 graphs by dequantizing their weights at load time; TEN and MarbleNet INT8 require `runtime="onnxruntime"`.
