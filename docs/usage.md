...
PulseVAD, TEN VAD, and MarbleNet use a recommended starting threshold of `0.5`, so specify `--threshold 0.5` when selecting these models. See [Models and runtimes](models-and-runtimes.md) for model downloads and runtime compatibility.
...
All runtime diagnostics use the `log` facade. Set `RUST_LOG` to control the CLI's
filter; model-specific details are emitted at the `debug` level. `--debug` is a
convenience shorthand that selects the `debug` filter only when `RUST_LOG` is
not set.
...
| `--debug` | Use debug-level logging when `RUST_LOG` is unset | off |
